<?php 
	...
	$db = new Database();
	$data = $db->get_data_json('SELECT id_kategori, nama_kategori FROM tbl_kategori');
?>
<div class="row">
	<div class="col-sm-12">
...
<div class="row">
	<div class="col-sm-12">
		<table class="table table-bordered table-stripe table-hover" id="tblKategori"></table>
	</div>
</div>
		...
		<form action="#" id="formAdd" method="post" class="form-horizontal">
			...
			<div class="modal-body">
				<div class="form-group">
					<label class="control-label col-sm-3" for="nama-add">Nama Kategori</label>
					<div class="col-sm-6">
...
<div class="modal fade" tabindex="-1" role="dialog" id="modalEdit">
	<div class="modal-dialog" role="document">
		...
		<form action="#" id="formEdit" method="post" class="form-horizontal">
			...
			<div class="modal-body">
				<div class="form-group">
					<label class="control-label col-sm-3" for="nama-edit">Nama Kategori</label>
					<div class="col-sm-6">
...
<script>
	$(document).ready(function(){
		var table = $('#tblKategori').DataTable({
			data: <?php echo $data; ?>,
			columns: [
		...
		$('#tblKategori tbody').on('click', '#edit', function(){
			var data = table.row($(this).parents('tr')).data();
			$('#kode-edit').val(data.id_kategori);
			$('#nama-edit').val(data.nama_kategori);
			$('#modalEdit').modal({ backdrop: 'static' });
		});

		$('#tblKategori tbody').on('click', '#delete', function(){
			var data = table.row($(this).parents('tr')).data();
			swal({
				title: "Hapus Data",
				text: "Hapus Data Kategori : " + data.nama_kategori + " ?",
				type: "warning",
			...
			function(isConfirm){
				if (isConfirm) {
					$.ajax({
						url: "pages/data-route.php?r=DeleteKategori",
						type: "POST",
						data: "kode-delete=" + data.id_kategori,
		...
		$('#formAdd').submit(function(e){
			$.ajax({
				url: "pages/data-route.php?r=AddKategori",
				type: "POST",
				data: $(this).serialize(),
				success: function(result){
					if(result === "1"){
		...
		$('#formEdit').submit(function(e){
			$.ajax({
				url: "pages/data-route.php?r=EditKategori",
				type: "POST",
				data: $(this).serialize(),
				success: function(result){
					if(result === "1"){
